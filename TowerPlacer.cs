...
public class TowerPlacer : MonoBehaviour
{
			...
			if (Physics.Raycast(ray, out hit))
			{
				...
				if (hitTransform != null && hitTransform.root.name == "BuildableWalls")
				{
					...
					if (!IsTowerTooClose(hit.point))
					{
						...
						GameObject newTower = Instantiate(towerPrefabs[towerPrefabIndex], hit.point, Quaternion.identity);
	...
	public void EnableTowerPlacement(int index)
	{
		towerPrefabIndex = index;
		if (GameController.Instance.gold < towerPrefabs[towerPrefabIndex].GetComponent<Tower>().towerStats.Price)
        {
			return;
        }
		canPlaceTower = true; // Allow the player to place a tower
		GameController.Instance.gold -= towerPrefabs[towerPrefabIndex].GetComponent<Tower>().towerStats.Price;
	}
